...
export function CartProvider({ children }: CartProviderProps): JSX.Element {
  const [cart, setCart] = useState<Product[]>(() => {
    const storagedCart = localStorage.getItem('@LojaLiven:cart');
  ...
  const addProduct = async (productId: number) => {
    try {
      ...
      if (productAlreadyExists) {
        ...
        const { data: stock } = await api.get(`products/${productId}`);

        const productIsAvailableInStock = stock.rating.count > productAmount;
  ...
  const removeProduct = (productId: number) => {
    try {
      ...
      localStorage.setItem('@LojaLiven:cart', JSON.stringify(filteredCart));
    } catch {
  ...
  const updateProductAmount = async ({
    ...
    try {
      ...
      const productIsAvailableInStock = stock.rating.count >= amount;
